...
class mod_hsuforum_mail_testcase extends advanced_testcase {
    ...
    public function setUp() {
        ...
        require_once($CFG->dirroot . '/mod/hsuforum/repository/discussion.php');
    ...
    /**
     * Perform message inbound setup for the mod_hsuforum reply handler.
     */
    protected function helper_spoof_message_inbound_setup() {
        ...
        $record = $DB->get_record('messageinbound_handlers', array('classname' => '\mod_hsuforum\message\inbound\reply_handler'));
        $record->enabled = true;
    ...
    protected function helper_update_subscription_time($user, $discussion, $factor) {
        return; // Our implementation doesn't have the concept of preference time.
        global $DB;
    ...
    protected function helper_run_cron_check_counts($posts, $expected) {
        ...
        // There should be the expected number of messages.
        $this->assertCount($expected, $messages);
    ...
    public function test_subscription_disabled() {
        ...
        $options = array('course' => $course->id, 'forcesubscribe' => HSUFORUM_DISALLOWSUBSCRIBE);
        $forum = $this->getDataGenerator()->create_module('hsuforum', $options);
        ...
        hsuforum_subscribe($recipient->id, $forum->id);

        $this->assertEquals($expected, $DB->count_records('hsuforum_subscriptions', array(
            'userid'        => $recipient->id,
            'forum'         => $forum->id,
        )));
        ...
        hsuforum_unsubscribe($recipient->id, $forum->id);
        ...
        $repo->subscribe($discussion->id, $recipient->id);
    ...
    public function test_automatic_with_unsubscribed_user() {
        ...
        $options = array('course' => $course->id, 'forcesubscribe' => HSUFORUM_INITIALSUBSCRIBE);
        $forum = $this->getDataGenerator()->create_module('hsuforum', $options);
        ...
        hsuforum_unsubscribe($author->id, $forum->id);
    ...
    public function test_optional_with_subscribed_user() {
        ...
        $options = array('course' => $course->id, 'forcesubscribe' => HSUFORUM_CHOOSESUBSCRIBE);
        $forum = $this->getDataGenerator()->create_module('hsuforum', $options);
    ...
    public function test_automatic_with_unsubscribed_discussion() {
        ...
        $repo = new \hsuforum_repository_discussion();
        $repo->unsubscribe($discussion->id, $author->id);

        $authorsubs = $repo->get_user_subscriptions($forum->id, $author->id);
        $this->assertEmpty($authorsubs);
        $recipientsubs = $repo->get_user_subscriptions($forum->id, $recipient->id);
        $this->assertEmpty($recipientsubs);

        // We expect only one user to receive this post.
        $expected = 1;
        // In hsuforum We expect two users to recieve this, as unsubscribing
        // from the discussion, does not overule the forum level subscription.
        $expected = 2;
        ...
        // Check we only saw one user.
        // $this->assertFalse($seenauthor);
        // In hsuforum, we will see the author, for same reason we set expected
        // to 2 earlier.
        $this->assertTrue($seenauthor);
        $this->assertTrue($seenrecipient);
    }
    ...
    public function test_optional_with_subscribed_discussion() {
        ...
        // you have read it.
        $expected = 0;
        // The previous comment doesn't make sense to me, but taking it at face
        // value, core forum won't let you subscribe to a discussion until
        // after you read a post from it. There's no such limit in hsuforum.
        $expected = 1;
    ...
    public function test_automatic_with_subscribed_discussion_in_unsubscribed_forum() {
        ...
        $repo = new \hsuforum_repository_discussion();
        $repo->subscribe($discussion->id, $author->id);
        ...
        $expected = 1;
        // In hsuforum, the subscription time is not checked, only the
        // subscription state at the time of mailing matters.
        $expected = 2;
    ...
    public function test_optional_with_unsubscribed_discussion_in_subscribed_forum() {
        ...
        // subscribe the 'recipient' user to the forum.
        hsuforum_subscribe($recipient->id, $forum->id);

        // Then unsubscribe them from the discussion.
        $repo = new \hsuforum_repository_discussion();
        $repo->unsubscribe($discussion->id, $recipient->id);

        // We don't expect any users to receive this post.
        $expected = 0;
        // In hsuforum, we respect the forum subscription, even if a thread
        // within the forum is unsubscribed from.
        $expected = 1;
    ...
    public function test_forum_discussion_subscription_forum_unsubscribed_discussion_subscribed_after_post() {
        ...
        $expectedcount = 1;
        // In hsuforum, subscription time is not relevant.
        $expectedcount = 2;
    ...
    public function test_forum_message_inbound_multiple_posts() {
        ...
        $author->emailstop = '0';
        set_user_preference('message_provider_mod_hsuforum_posts_loggedoff', 'email', $author);
        set_user_preference('message_provider_mod_hsuforum_posts_loggedin', 'email', $author);
    ...
    public function test_long_subject() {
        ...
        $expectedsubject = get_string('postmailsubject', 'hsuforum', $a);
    ...
    public function test_forum_post_email_templates($data) {
        ...
        foreach ($data['forums'] as $dataforum) {
            ...
            foreach ($forumposts as $forumpost) {
                ...
                if ($attachments) {
                    ...
                    $DB->set_field('hsuforum_posts', 'attachment', '1', array('id' => $post->id));
                }
            }
        }
        ...
        hsuforum_cron(); // It's really annoying that we have to run cron to test this.
